...
function pure_portfolio_render_gallery_section( $args ) {
	$section_title = get_theme_mod( 'pure_portfolio_gallery_title', __( 'Our Gallery', 'impressive-portfolio' ) );
	$section_text  = get_theme_mod( 'pure_portfolio_gallery_text', '' );
	$button_label  = get_theme_mod( 'pure_portfolio_gallery_button_label', __( 'View All', 'impressive-portfolio' ) );
	$button_link   = get_theme_mod( 'pure_portfolio_gallery_button_link', '' );

	$query = new WP_Query( $args );
	if ( $query->have_posts() ) :
		?>
		<section id="pure_portfolio_gallery_section" class="pure-portfolio-frontpage-section pure-portfolio-secondary1-background pure-portfolio-gallery-section gallery-style-1">
			<?php
