...
function pure_portfolio_render_blog_section( $args ) {
	$section_title     = get_theme_mod( 'pure_portfolio_blog_title', __( 'Recent News', 'impressive-portfolio' ) );
	$section_text      = get_theme_mod( 'pure_portfolio_blog_text', '' );
	$post_button_label = get_theme_mod( 'pure_portfolio_excerpt_more_text', __( 'Read More', 'impressive-portfolio' ) );
	$button_label      = get_theme_mod( 'pure_portfolio_blog_button_label', __( 'View All', 'impressive-portfolio' ) );
	$button_link       = get_theme_mod( 'pure_portfolio_blog_button_link', '' );

	$query = new WP_Query( $args );
	if ( $query->have_posts() ) :
		?>
		<section id="pure_portfolio_blog_section" class="pure-portfolio-frontpage-section pure-portfolio-blog-section blog-style-1">
			<?php
